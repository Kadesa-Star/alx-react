...
describe('<Notifications />', () => {
        ...
        // New test to ensure no duplicate notifications when updating listNotifications
        it(`Tests that when the props listNotifications are updated, no duplicates are created`, () => {
                const listNotificationsWithDuplicates = [
                        { id: 1, type: "default", value: "New course available" },
                        { id: 2, type: "urgent", value: "New resume available" },
                        { id: 1, type: "default", value: "New course available" }, // duplicate
                ];

                const wrapper = shallow(<Notifications displayDrawer listNotifications={listNotificationsWithDuplicates} />);
                wrapper.update();

                const notificationItems = wrapper.find(NotificationItem);
                expect(notificationItems.length).toBe(2); // No duplicates should be created
        });
});
