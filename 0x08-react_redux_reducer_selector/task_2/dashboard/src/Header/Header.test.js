...
// Helper function to wrap components in the context provider
const wrapWithContext = (value) => {
  return mount(
...
describe('<Header />', () => {
  ...
  it('Tests that the component displays login message when not logged in', () => {
    ...
    const wrapper = wrapWithContext(contextValue);
  ...
  it('Tests that the component displays logout message when logged in', () => {
    const contextValue = {
      user: { email: 'user@example.com', isLoggedIn: true },
      logOut: jest.fn(),
    };
    const wrapper = wrapWithContext(contextValue);
