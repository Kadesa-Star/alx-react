...
import { filterTypeSelected, getNotifications, getUnreadNotifications, getUnreadUrgentNotifications } from './notificationSelector';
...
describe('Notification Selectors', () => {
  ...
  it('should return only unread urgent notifications when the filter is set', () => {
    const urgentState = initialState.set('filter', NotificationTypeFilters.URGENT);
    const unreadUrgentNotifications = getUnreadUrgentNotifications(urgentState);
    const expectedUnreadUrgentNotifications = fromJS({
      3: { id: 3, type: 'urgent', value: 'New data available', isRead: false },
    });

    expect(unreadUrgentNotifications).toEqual(expectedUnreadUrgentNotifications);
  });
});
